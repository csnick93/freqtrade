...
class Obelisk_Ichimoku_ZEMA_v1(IStrategy):
    ...
    low_offset = DecimalParameter(0.80,
                                  1.20,
                                  default=1.004,
                                  space='buy',
                                  optimize=True)
    high_offset = DecimalParameter(0.80,
                                   1.20,
                                   default=0.964,
                                   space='sell',
                                   optimize=True)
    zema_len_buy = IntParameter(30, 90, default=72, space='buy', optimize=True)
    zema_len_sell = IntParameter(30,
                                 90,
                                 default=51,
                                 space='sell',
                                 optimize=True)
    ...
    def fast_tf_indicators(self, dataframe: DataFrame,
                           metadata: dict) -> DataFrame:
        if self.config['runmode'].value == 'hyperopt':
            ...
            dataframe['zema_buy'] = ftt.zema(
                dataframe,
                period=self.zema_len_buy.value) * self.low_offset.value
            dataframe['zema_sell'] = ftt.zema(
                dataframe,
                period=self.zema_len_sell.value) * self.high_offset.value
    ...
    def populate_indicators(self, dataframe: DataFrame,
        ...
        if self.timeframe == self.informative_timeframe:
            ...
            dataframe.rename(columns=lambda s: s.replace(
                ...
                if (s not in skip_columns) else s,
                inplace=True)

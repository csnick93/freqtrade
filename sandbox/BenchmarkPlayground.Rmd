...
def read_ticker_data(ticker_file):
    ...
    indicator_df = (indicator_df - indicator_df.min()) / (indicator_df.max() - indicator_df.min() + 1e-5)
...
def create_samples(ticker_df) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for i in range(int((ticker_df.shape[0] - 2*SAMPLE_LENGTH)/ SAMPLE_STEPSIZE)):
        ...
        samples.append(sample[['close', 'high', 'low', 'open', 'volume', 'zema_72', 'zema_51',
                               ...
                               'ichimoku_ok', 'ichimoku_bear', 'trend_pulse',
                               'bear_trend_pulse', 'trend_over', 'bear_trend_over', 'trending',
                               'bear_trending']].values)
        labels.append(label)
        fps.append(future_prices.values)
        
    return np.asarray(samples), np.asarray(labels), np.asarray(fps)
```
...
```{python}
samples, labels, future_prices = create_samples(ticker_df)
```
...
for i in tqdm(range(samples.shape[0])):
    sample_price = samples[i, -1, 0]
    future_max = np.max(future_prices[i])
    label = labels[i]
    
    perc_change = (future_max - sample_price) / sample_price
    
    if label == 1 and perc_change < INCREASE_TARGET:
        raise ValueError(f'Faulty label for sample {i}')
    elif label == 0 and perc_change > INCREASE_TARGET:
        raise ValueError(f'Faulty label for sample {i}')
...
for ticker_file in tqdm(os.listdir(root_dir)):
    try:
        ...
        samples, labels, _ = create_samples(ticker_df)
...
# Compute Obelisk Strategy accuracy

```{python}
def compute_obelisk_label(sample):
    np.where((sample[:, 31] > 0) & (samples[:, 37]==0) & (samples[:,0] < samples[:, 6] * 1.004))
```

```{python}
ticker_df['buy'] = 0
ticker_df.loc[(ticker_df['ichimoku_valid'] > 0)
                      & (ticker_df['bear_trending'] == 0)
                      & (ticker_df['close'] <
                         (ticker_df['zema_51'] * 1.004)), 'buy'] = 1
```

```{python}
sample=samples[0]
result = np.where((sample[:, 31] > 0) & (sample[:, 36]==0) & (sample[:,0] < sample[:, 6] * 1.004))
```
